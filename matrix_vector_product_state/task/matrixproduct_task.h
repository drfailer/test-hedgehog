...
class MatrixProductTask: public hh::AbstractTask<1, ProductBlockData<Type>, VectorBlockData<Type, Result>> {
  public:
    MatrixProductTask(std::string const& name, size_t nbThreads):
        hh::AbstractTask<1, ProductBlockData<Type>, VectorBlockData<Type, Result>>(name, nbThreads) {}
    ...
    void execute(std::shared_ptr<ProductBlockData<Type>> productData) override {
        ...
        for (size_t i = ibegin; i < iend; ++i) {
            for (size_t j = jbegin; j < jend; ++j) {
                resultData[i] += matrixData[i * matrixWidth + j] * vectorData[j]; // need lock ?
            }
        }
        this->addResult(resultBlocData);
    }

    std::shared_ptr<hh::AbstractTask<1, ProductBlockData<Type>, VectorBlockData<Type, Result>>> copy() override {
